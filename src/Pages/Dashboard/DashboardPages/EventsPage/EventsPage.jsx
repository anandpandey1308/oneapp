...
const EventsPage = () => {
  ...
  return (
    <div className="min-h-screen">
      {/* Background Section */}
      <div className="w-full h-64 bg-gradient-to-b from-[#ADD8E6] to-[#FFD700] flex justify-center items-center relative">
        <h1 className="font-bold text-white text-3xl md:text-4xl">Event Page</h1>
        <button
          type="button"
      ...
      <div className="p-6 h-full w-full flex items-center justify-center">
        {tabContent[activeTab].content ? (
          <>
            {activeTab === 0 && (
              <div className="bg-blue-100 text-blue-600 p-4 rounded-lg">
                <h2 className="font-bold text-lg mb-2">Published Events</h2>
                <p>{tabContent[activeTab].content}</p>
              </div>
            )}
            {activeTab === 1 && (
              <div className="bg-purple-100 text-purple-600 p-4 rounded-lg">
                <h2 className="font-bold text-lg mb-2">Unpublished Events</h2>
                <p>{tabContent[activeTab].content}</p>
              </div>
            )}
            {activeTab === 2 && (
              <div className="bg-orange-100 text-orange-600 p-4 rounded-lg">
                <h2 className="font-bold text-lg mb-2">Draft Events</h2>
                <p>{tabContent[activeTab].content}</p>
              </div>
            )}
          </>
          ...
          <NoContentComponent
            title={noContentData.events[activeTab].title}
            description={noContentData.events[activeTab].description}
            isbutton={noContentData.events[activeTab].isbutton}
            button_title={noContentData.events[activeTab].button_title}
          />
        )}
      </div>
    </div>
